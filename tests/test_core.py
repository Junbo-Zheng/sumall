...
def test_parse_line_excludes_version_date_negative():
...
def test_parse_line_percent_values():
    assert parse_line("30%").value == 30
    assert parse_line("30%").unit == "%"
    # inside a row, a bare value cell beats the % column (heuristic picks)
    result = parse_text("ap 13.5 30%\n")
    assert [e.value for e in result.entries] == [13.5]
    assert result.multi == 1
    # percent-only rows sum as raw values
    result = parse_text("46.4%\n14.1%\n")
    assert [e.value for e in result.entries] == [46.4, 14.1]
    assert result.multi == 0
