...
def test_args_sum_with_breakdown_and_totals(capsys):
    ...
    # all bare numbers, no --unit: raw sum, no unit conversion
    assert "input: 4 values" in out
...
def test_ls_l_paste_end_to_end(capsys, monkeypatch):
    ...
    assert main([]) == 0
    out = capsys.readouterr().out
    assert "input: 4 values, unit mixed (3 lines skipped)" in out
...
def test_markdown_table_paste(capsys, monkeypatch):
    ...
    assert "input: 3 values, unit mixed (2 lines skipped)" in out
    assert "multiple numeric columns" not in out
...
def test_df_h_paste_multi_column_heuristic(capsys, monkeypatch):
    ...
    assert main([]) == 0
    out = capsys.readouterr().out
    assert "multiple numeric columns" in out  # heuristic fired; the note teaches -c
...
def test_percent_only_paste_sums_as_percent(capsys, monkeypatch):
    monkeypatch.setattr(sys, "stdin", io.StringIO("46.4%\n14.1%\n9.1%\n"))
    assert main([]) == 0
    out = capsys.readouterr().out
    assert "unit %" in out
    assert "69.60 %" in out
    assert "MB" not in out  # percentages are not sizes; no byte conversion
...
def test_column_flag_overrides_heuristic(capsys, monkeypatch):
    ...
    assert main(["-c", "2"]) == 0
    out = capsys.readouterr().out
    assert "19.50" in out  # 13.5 + 6, the second column, bare -> unitless
    assert "MB" not in out
    assert "multiple numeric columns" not in out
