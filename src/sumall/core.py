...
# Binary (MiB-style) units, matching du/df -h output. "%" is not a size:
# its factor is 1 so percent entries keep their raw value; the CLI prints a
# percent-only total without MB/KB/bytes conversion.
UNIT_BYTES = {
    "B": 1,
    "KB": 1024,
    "MB": 1024 * 1024,
...
# with optional i/B as in KiB/GB, a bare B, or a percent sign). Versions,
...
class Entry:
    ...
    # True when the input carried no unit suffix: a plain number, not a
    # size. The CLI sums unitless when every entry is bare and no --unit
    # was given; a unit-marked neighbor (or --unit) anchors bare values to
    # a size unit instead.
    bare: bool = False
...
def _entry(m: re.Match, unit: str, label: str, shown: str, bare: bool = False) -> Entry:
    value = float(m.group("value").replace(",", ""))
    line = f"{shown} {label}" if label else shown
    return Entry(value=value, unit=unit, label=label, line=line, bare=bare)
...
def _parse_cells(
    ...
    if column is not None:
        ...
        # Prefer a size-marked cell (13.5MB beats 120), then a bare number,
        # then a percent cell (df -h pastes: 757G beats 13, and a value
        # column beats a trailing Use% column); ties take the last cell —
        # size columns usually sit at the end of a row.
        u = lambda p: p[1].group("unit")  # noqa: E731
        sized = [p for p in matches if u(p) and u(p) != "%"]
        bare = [p for p in matches if not u(p)]
        pct = [p for p in matches if u(p) == "%"]
        idx, m = (sized or bare or pct)[-1]
        multi = True
    raw_unit = m.group("unit")
    ...
    return _entry(m, unit, label, cells[idx], bare=raw_unit is None), multi
...
def parse_token(token: str, default_unit: str = "MB") -> Entry:
    ...
    raw_unit = m.group("unit")
    unit = _norm_unit(raw_unit) if raw_unit else default_unit
    return _entry(m, unit, "", token, bare=raw_unit is None)
...
def describe_unit(entries: list[Entry], default_unit: str) -> str:
    """The default unit name when every entry uses it, '%' when every entry
    is a percentage, else 'mixed'."""
    units = {e.unit for e in entries}
    if units == {"%"}:
        return "%"
    return default_unit if units == {default_unit} else "mixed"
